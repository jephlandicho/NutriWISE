...
const AllMealPlan = ({ exchange_id }) => {
  ...
  const handleExchange = () => {
    console.log(`Exchange with ID ${exchange_id} initiated`);
  ...
  return (
    <TouchableOpacity style={styles.meabutton} onPress={handleSaveasPDF(exchange_id)}>
    <Text style={styles.buttonText}>
      <Ionicons name="md-save-outline" size={30} color="black" />
    </Text>
  </TouchableOpacity>
  );
};
